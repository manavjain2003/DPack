...
// Cart works fully offline (localStorage) for guests. Whenever a session
// token exists, mutations are also mirrored to the server in the
// background, best-effort, so logged-in users get a cart that persists
// across devices. Local state is always the source of truth for the UI.
function isLoggedIn() {
...
function backgroundSync(fn) {
  ...
  fn().catch(() => {}); // best-effort; local cart already reflects the change
}
...
// Call right after login (and on initial hydrate if a token already
// exists) to reconcile the guest's local cart with the server's cart.
// Overlapping items take the larger quantity server-side, so this is
// safe to call repeatedly without double-counting.
